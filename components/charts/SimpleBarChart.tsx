...
export const SimpleBarChart: React.FC<SimpleBarChartProps> = ({ data, currency }) => {
    ...
    return (
        <div style={{ padding: '0 0.5rem', color: 'var(--color-text)', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            {data.map((item, index) => {
                ...
                return (
                    <div key={index} style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', alignItems: 'center', gap: '1rem', fontSize: '0.9rem' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                             <div style={{ width: '100%', backgroundColor: '#e5e7eb', borderRadius: '4px', height: '24px', position: 'relative' }}>
                                <div
                                    style={{ 
                                        position: 'absolute',
                                        height: '100%', 
