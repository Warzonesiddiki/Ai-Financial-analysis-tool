...
export const SimpleLineChart: React.FC<SimpleLineChartProps> = ({ data, currency }) => {
  ...
  const handleMouseMove = (e: React.MouseEvent, content: string) => {
    ...
    setTooltip({ content, x: rect.left + window.scrollX + rect.width / 2, y: rect.top + window.scrollY });
  };
  const handleMouseLeave = () => setTooltip(null);

  return (
    <div style={{ color: 'var(--color-text)', position: 'relative' }}>
      {tooltip && (
      ...
      <svg viewBox={`0 0 ${width} ${height}`} style={{ width: '100%', height: 'auto' }}>
        {effectiveRange !== 0 && yAxisLabels.map((label, i) => (
          <g key={i} style={{ fontSize: '12px', color: 'var(--color-text-secondary)' }}>
            <line x1={padding.left} y1={yScale(label)} x2={width - padding.right} y2={yScale(label)} stroke="var(--color-border)" strokeDasharray="2,3" strokeWidth="1" />
            <text x={padding.left - 10} y={yScale(label) + 4} textAnchor="end" fill="currentColor">{formatYLabel(label)}</text>
          </g>
        ))}

        {labels.map((label, i) => (
          <g key={i} style={{ fontSize: '12px', color: 'var(--color-text-secondary)' }}>
             <text x={xScale(i)} y={height - padding.bottom + 20} textAnchor="middle" fill="currentColor">{label}</text>
          </g>
        ))}

        {seriesNames.map((seriesName, seriesIndex) => {
          const points = seriesData[seriesName].map(d => { const index = labels.indexOf(d.label); if(index === -1) return null; return `${xScale(index)},${yScale(d.value)}`; }).filter(Boolean).join(' ');
          const color = COLORS[seriesIndex % COLORS.length];

          return (
            <g key={seriesName}>
              {labels.length > 1 && <polyline fill="none" stroke={color} strokeWidth="2.5" points={points} />}
              {seriesData[seriesName].map(d => {
                const index = labels.indexOf(d.label);
                if (index === -1) return null;
                const tooltipContent = `${seriesName !== 'default' ? seriesName + ': ' : ''}${formatValue(d.value)}`;
                return (
                  <circle
         ...
         <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '1rem 1.5rem', marginTop: '1rem', fontSize: '0.875rem' }}>
            {seriesNames.map((seriesName, seriesIndex) => (
              <div key={seriesName} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                <span style={{ width: '12px', height: '12px', borderRadius: '4px', backgroundColor: COLORS[seriesIndex % COLORS.length] }}></span>
