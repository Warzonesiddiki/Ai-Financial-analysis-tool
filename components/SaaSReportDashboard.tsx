...
import { SaaSNarrativeResponse, SaaSReportData, SaaSReportSection, SectionAnalysis } from '../types';
import { Spinner } from './Spinner';
import { StatCard } from './StatCard';
import { ArrowLeftIcon, AlertTriangleIcon, CheckCircleIcon, XIcon } from './icons';
import { SAAS_REPORT_SECTIONS } from '../constants';
...
interface SaaSReportDashboardProps {
    narrative: SaaSNarrativeResponse | null;
...
const SaaSReportDashboard: React.FC<SaaSReportDashboardProps> = ({ narrative, reportData, isLoading, progress, onBackToInput }) => {
    const [activeSectionId, setActiveSectionId] = useState<string>(SAAS_REPORT_SECTIONS[0].id);

    const activeSectionData = useMemo(() => {
        return narrative?.sections.find(s => s.id === activeSectionId);
    }, [narrative?.sections, activeSectionId]);

     const renderCharts = (section: SaaSReportSection | undefined) => {
            ...
            <div className={`grid ${chartLayout}`} style={{gap: '1.5rem'}}>
                {section.analysis.charts.map((chart, index) => (
                    <div key={index} className="card" id={`chart-${section.id}-${index}`} style={{ gridColumn: section.analysis.charts.length === 1 ? 'span 2' : 'span 1'}}>
                         ...
                         {/* Waterfall chart would need a custom component */}
                         {chart.type === 'waterfall' && <SimpleBarChart data={chart.data} currency={reportData.currency} />}
                    </div>
                ))}
            </div>
        );
    ...
    if (isLoading && Object.values(progress).every(s => s === 'loading' || s === 'pending')) {
        return (
            <div className="card" style={{textAlign: 'center', padding: '4rem', minHeight: '50vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center'}}>
                <Spinner />
                <h3 style={{marginTop: '1rem'}}>Analyzing SaaS Metrics...</h3>
    ...
    return (
        <div id="report-container">
            <header style={{marginBottom: '1.5rem'}}>
                 ...
                 <p style={{color: 'var(--color-text-secondary)', marginTop: '0.25rem', fontSize: '1.1rem'}}>SaaS & Subscription Analysis</p>
            </header>
            ...
            <div className="dashboard">
                <nav className="dashboard-nav">
                     <div className="card" style={{padding: '0.75rem'}}>
                        ...
                        {SAAS_REPORT_SECTIONS.map(section => {
...
export default SaaSReportDashboard;